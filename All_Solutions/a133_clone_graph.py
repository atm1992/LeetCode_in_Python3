...
class Solution:
    ...
    def cloneGraph(self, node: 'Node') -> 'Node':
        ...
        # 这里使用node.val作为key，而不是使用node作为key，是因为已知node.val的类型为int(不可变hashable, 且不重复)，而node的类型不一定是不可变的。dict要求key必须为不可变hashable
